...
3. After explicit cwd/worktree validation, launch only the validated profile command and argv on the explicitly recorded new executor surface in the `cmux-agent` workspace. Keep the recorded workspace and surface as the routing identity. For Cursor, launch the original interactive `agent --trust` profile only; establish the supervisor-owned mapping, fresh source boundary, additive hook bridge, and explicitly installed watcher before sending the job prompt. Export the per-job runtime on the dedicated `CMUX_AGENT_JOB_RUNTIME` variable as well as the common mapping; Cursor overwrites `CMUX_AGENT_RUNTIME` for its own project state inside hooks. Never modify or silently install user-level hooks or watcher binaries; the bridge creates a fresh per-job transcript/result boundary from the hook-provided path.
...
5. Monitor with push + pull: watch the profile-declared hook-provided transcript/result and bridge-event sources for fresh, correlated events and exact assistant markers, and periodically read the explicit executor surface to classify the executor as `idle`, `working`, `question`, or `lost`. Hooks are wakeups only. `stop` and `afterAgentResponse` are optional; stop `error`/`aborted` fails closed, while missing stop never weakens transcript validation. Do not treat screen text, prompt echo, a hook event, or an exit code alone as completion. If the optional advisor is configured, invoke it only through the watcher after the fifteen-second quiet threshold and its capped 15/30/60-second backoff; new transcript/event activity resets that backoff.
...
7. Relay `NEED_APPROVAL` and `QUESTION` verbatim to the main agent and wait for its explicit response. A strict advisor recommendation may proceed automatically only for an exact displayed command independently validated as a routine read-only local command under `routine-command-v1`; destructive, credential, deployment, external-network, ambiguous, or important decisions (including trust/authorization, authentication, spending, irreversible, or scope-expanding work) always escalate. Advisor failure is fail-closed. Never approve consequential work or answer from guesswork; do not guess an answer. Keep the executor tool-free at the `NEED_APPROVAL` checkpoint, send only the main agent's response on the explicit executor surface, and resume with a fresh bounded deadline.
