...
| `tools/cmux-agent-wait.py` | Standalone atomic result poller with nonce validation and deadline/grace fence. |
| `tools/cmux-agent-setup.sh` | Explicit plan/apply/check installation of profiles, runner, waiter, and runtime directories. |
...
| `tests/cmux-agent-run-contract.sh` | Runner lifecycle, timeout, timing metadata, and shell-safety fixtures. |
| `tests/cmux-agent-wait-contract.sh` | Deterministic result polling, identity, terminal-state, and fence fixtures. |
| `tests/cmux-agent-contract.sh` | Headless worker, cmux routing, waiter, and finalization contract checks. |
...
4. The runner captures process evidence without parsing provider transcripts or
   screen text.
5. The installed waiter polls only the runner-owned `result.json` through its
   deadline + stop-grace + safety-margin fence.
6. The worker checks declared artifacts and focused checks, then reports paths
   and hashes before reserving the final 2–5 minutes for durable finalization.
7. The calling agent independently reviews the worktree and repeats important
   checks.
...
the common runner, result waiter, and runtime directories. It does not
...
bash tests/cmux-agent-wait-contract.sh
