...
Correlation is split by ownership. The hook supplies the invocation identity: `event_id`, `executor_session`, `conversation_id`, `transcript_path`, and `transcript_offset` (the result-file byte offset where this invocation's fresh segment starts). The supervisor owns `job_nonce`, `workspace`, `surface`, and `cwd` through the active-job mapping recorded before launch, and binds the hook identity to that mapping; it never expects them inside the hook payload. Deduplication uses the profile's `deduplicate_by` identity — `event_id` plus `executor_session` — both stable across a replayed notification of the same invocation, so a replay re-presents the same declared identity and is deduplicated rather than double-counted. `transcript_offset` is deliberately **not** part of the dedupe identity: it is fresh-segment metadata that advances on every accepted capture by design, and folding it into the identity would make replays look like new events.
